$(document).ready(function() {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/fix_it/fetch_fix_it',
            data: {
...
crud.set_after_update = function (items,row_id) {
    ...
    row_id.find("td:eq(0)").html(items.id);row_id.find("td:eq(1)").html(items.date_fix);row_id.find("td:eq(2)").html(items.detail);row_id.find("td:eq(3)").html(items.com_id);row_id.find("td:eq(4)").html(items.fix_status);row_id.find("td:eq(5)").html(items.provider);

}
crud.set_after_insert = function (items,id) {

            $('<tr name="row'+(id+1)+'"><td>'+id+'</td>' +
                '<td>' +items.id+'</td>' +'<td>' +items.date_fix+'</td>' +'<td>' +items.detail+'</td>' +'<td>' +items.com_id+'</td>' +'<td>' +items.fix_status+'</td>' +'<td>' +items.provider+'</td>' +
...
crud.set_update = function (data,row_id) {
    $("#row_id").val(row_id);
    $("#id").val(data.rows["id"]);$("#date_fix").val(data.rows["date_fix"]);$("#detail").val(data.rows["detail"]);$("#com_id").val(data.rows["com_id"]);$("#fix_status").val(data.rows["fix_status"]);$("#provider").val(data.rows["provider"]);
}
...
$('#btn_save').on('click', function (e) {
    ...
    items.id=$("#id").val();items.date_fix=$("#date_fix").val();items.detail=$("#detail").val();items.com_id=$("#com_id").val();items.fix_status=$("#fix_status").val();items.provider=$("#provider").val();
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุID");$("#id").focus();}else if (!items.date_fix) { swal("กรุณาระบุวันที่ซ่อม");$("#date_fix").focus();}else if (!items.detail) { swal("กรุณาระบุอาการ");$("#detail").focus();}else if (!items.com_id) { swal("กรุณาระบุอุปกรณ์ที่ซ่่อม");$("#com_id").focus();}else if (!items.fix_status) { swal("กรุณาระบุสภานะการซ่อม");$("#fix_status").focus();}else if (!items.provider) { swal("กรุณาระบุ");$("#provider").focus();}
    else{
        return true;
    }
