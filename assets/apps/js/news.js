...
crud.set_after_update = function (items,row_id) {
    ...
    row_id.find("td:eq(0)").html(items.id);row_id.find("td:eq(1)").html(items.topic);row_id.find("td:eq(2)").html(items.detail);row_id.find("td:eq(3)").html(items.date_sent);row_id.find("td:eq(4)").html(items.user_id);row_id.find("td:eq(5)").html(items.cat_id);row_id.find("td:eq(6)").html(items.read);row_id.find("td:eq(7)").html(items.files);
...
crud.set_update = function (data,row_id) {
    $("#row_id").val(row_id);
    $("#id").val(data.rows["id"]);$("#topic").val(data.rows["topic"]);$("#detail").val(data.rows["detail"]);$("#date_sent").val(data.rows["date_sent"]);$("#user_id").val(data.rows["user_id"]);$("#cat_id").val(data.rows["cat_id"]);$("#read").val(data.rows["read"]);$("#files").val(data.rows["files"]);
}
...
$('#btn_save').on('click', function (e) {
    ...
    items.id=$("#id").val();items.topic=$("#topic").val();items.detail=$("#detail").val();items.date_sent=$("#date_sent").val();items.user_id=$("#user_id").val();items.cat_id=$("#cat_id").val();items.read=$("#read").val();items.files=$("#files").val();
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุID");$("#id").focus();}else if (!items.topic) { swal("กรุณาระบุหัวข้อ");$("#topic").focus();}else if (!items.detail) { swal("กรุณาระบุรายละเอียด");$("#detail").focus();}else if (!items.date_sent) { swal("กรุณาระบุวันที่ส่ง");$("#date_sent").focus();}else if (!items.user_id) { swal("กรุณาระบุผู้ส่ง");$("#user_id").focus();}else if (!items.cat_id) { swal("กรุณาระบุหมวดหมู่");$("#cat_id").focus();}else if (!items.read) { swal("กรุณาระบุจำนวนการอ่าน");$("#read").focus();}else if (!items.files) { swal("กรุณาระบุไฟลล์");$("#files").focus();}
    else{
        return true;
    }
