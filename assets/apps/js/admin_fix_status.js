$(document).ready(function() {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/admin_fix_status/fetch_admin_fix_status',
            data: {
...
crud .ajax = {
    del_data:function (id,cb){
        var url = '/admin_fix_status/del_admin_fix_status',
            params = {
                id: id
            }
        ...
        app.ajax(url, params, function (err, data) {
            err ? cb(err) : cb(null, data);
             ...
             var url = '/admin_fix_status/save_admin_fix_status',
                 params = {
                     items: items
                 }
             ...
             app.ajax(url, params, function (err, data) {
                 err ? cb(err) : cb(null, data);
                   ...
                   var url = '/admin_fix_status/get_admin_fix_status',
                       params = {
                           id: id
                       }
...
crud.set_after_update = function (items,row_id) {
    ...
    row_id.find("td:eq(0)").html(items.id);row_id.find("td:eq(1)").html(items.name);

}
crud.set_after_insert = function (items,id) {

            $('<tr name="row'+(id+1)+'"><td>'+id+'</td>' +
                '<td>' +items.id+'</td>' +'<td>' +items.name+'</td>' +
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุID");$("#id").focus();}else if (!items.name) { swal("กรุณาระบุสถานะการซ่อม");$("#name").focus();}
    else{
        return true;
    }
