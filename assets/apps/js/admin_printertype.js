$(document).ready(function() {
    var dataTable = $('#table_data').DataTable({
        ...
        "pageLength": 50,
        "ajax": {
            url: site_url + '/admin_printertype/fetch_admin_printertype',
            data: {
...
crud .ajax = {
    del_data:function (id,cb){
        var url = '/admin_printertype/del_admin_printertype',
            params = {
                id: id
            }
        ...
        app.ajax(url, params, function (err, data) {
            err ? cb(err) : cb(null, data);
             ...
             var url = '/admin_printertype/save_admin_printertype',
                 params = {
                     items: items
                 }
             ...
             app.ajax(url, params, function (err, data) {
                 err ? cb(err) : cb(null, data);
                   ...
                   var url = '/admin_printertype/get_admin_printertype',
                       params = {
                           id: id
                       }
...
function validate(items){
    ...
    if (!items.id) { swal("กรุณาระบุID");$("#id").focus();}else if (!items.name) { swal("กรุณาระบุPrinter type");$("#name").focus();}
    else{
        return true;
    }
